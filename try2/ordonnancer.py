...
class Ordonnancer:
    ...
    def evaluateProject(self):
        for project in self.listProject:
            heappush(self.order, (self.calculerScore(project, self.listContributor), project))
    
    def createDistribution(self):
        _, project = heappop(self.order)
        self.distributionTaches.append(project.selectContributor(self.listContributor))